    ...
    if(!isnan(current_diff) && !isnan(last_diff)) {
      ...
      if(crossed && current_diff < kEdgeThreshold) {
        if(last_heartbeat != 0 && crossed_time - last_heartbeat > 300) {
         ...
         //GSM function call for messaging the data to Patient
    }
  }
...
void Buzzer()
{
  ...
  //GSM function call for calling doctor
  }
  val = 0;
}
...
void sendMessage(String msg)
{
  ...
  gsmSerial.println("AT+CMGS=\"+8801610920878\"\r"); // Replace x with mobile number
  delay(1000);
